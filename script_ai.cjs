...
const TOOLS = [
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          collection: { type: 'string', description: 'Nombre de la colección (ej. transactions, debts, accounts, external_cards, subscriptions)' },
          action:     { type: 'string', enum: ['create', 'update', 'delete'], description: 'La operación a realizar' },
          targetId:   { type: 'string', description: 'ID del documento (requerido para update o delete)' },
          payload:    { type: 'string', description: 'JSON string de los campos a mutar o crear. Ej: "{\\"monto\\": 500, \\"estado\\": \\"activo\\"}"' }
        },
  ...
  if (toolCall.type === 'execute_app_action') {
    ...
    try {
      if (action === 'create') {
        ...
        logAdminAction(userId, 'SYSTEM', \`[IA] Creación de registro en \${collection}\`, JSON.stringify(record));
        return { success: true, id, message: \`✅ He creado el nuevo registro en \${collection}.\` };
      } 
      else if (action === 'update' && targetId) {
        store.update(collection, targetId, dataObj);
        logAdminAction(userId, 'SYSTEM', \`[IA] Actualización en \${collection} (\${targetId})\`, JSON.stringify(dataObj));
        return { success: true, message: \`✅ Registro actualizado en \${collection}.\` };
      }
      else if (action === 'delete' && targetId) {
        store.remove(collection, targetId);
        logAdminAction(userId, 'SYSTEM', \`[IA] Eliminación en \${collection} (\${targetId})\`, '');
        return { success: true, message: \`✅ Eliminación ejecutada en \${collection}.\` };
      }
    } catch(e) {
  ...
  if (choice.finish_reason === 'tool_calls' && choice.message.tool_calls?.length > 0) {
    ...
    try {
      ...
      if (execResult.askedConfirmation) {
        ...
        message = execResult.message || \`Acción completada con éxito.\`;
      } else {
        message = \`Falló la ejecución: \${execResult.error}\`;
      }
      
      actionRes = actionPayload;

    } catch (e) {
      message = 'Detecté una acción pero hubo un error de parseo al procesarla.';
    }
  } else {
...
// Fallback logic not strictly needed since new prompt is so omnipotent, but leaving for signature compat
`;

const fileUri = new URL('file:///' + path.join(__dirname, 'src/ai-agent.js').replace(/\\/g, '/')).pathname;
fs.writeFileSync(path.join(__dirname, 'src/ai-agent.js'), fileContent, 'utf8');
console.log('ai-agent.js has been rewritten successfully.');
