...
REGLAS DE ORO:
1. Las transacciones deben incluir SIEMPRE "fecha" (YYYY-MM-DD). Si el usuario no dice cuándo, usa la fecha de hoy (${today}).
2. Nunca modifiques el "saldoInicial" de una cuenta 'accounts' para registrar un gasto; usa SIEMPRE la colección 'transactions'.
...
export function executeAction(toolCall) {
  ...
  if (toolCall.type === 'execute_app_action') {
    ...
    try {
      if (action === 'create') {
        // Enforce data integrity for transactions
        if (collection === 'transactions') {
          if (!dataObj.fecha) {
            const today = new Date().toISOString().split('T')[0];
            dataObj.fecha = today;
          }
          dataObj.source = 'ai-agent';
        }
...
/**
 * Valida si una API Key es funcional con una llamada mínima.
 */
export async function validateOpenAIKey(apiKey) {
  if (!apiKey) return { success: false, error: 'API Key vacía' };
  ...
  try {
    ...
    const errData = await response.json().catch(() => ({}));
    return { success: false, error: errData.error?.message || `Error HTTP ${response.status}` };
  } catch (err) {
